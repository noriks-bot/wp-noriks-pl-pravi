...
function gck_get_bundle_offers( $product_id = null ) : array {
   ...
   // var_dump($rows);
    ...
    foreach ( $rows as $row ) {
        ...
        $saving_text   = "Ukupna ušteda " . number_format($saving_amount, 2, '.', '') . " zł";
...
$is_color = ( strpos($hay, 'strona') !== false || strpos($hay, 'boja') !== false || strpos($hay, 'barva') !== false || strpos($hay, 'farb') !== false || strpos($hay, 'culoare') !== false || strpos($hay, 'color') !== false || strpos($hay, 'colour') !== false || strpos($hay, 'colore') !== false || strpos($hay, 'kolor') !== false || strpos($hay, 'szín') !== false || strpos($hay, 'szin') !== false || strpos($hay, 'ρώμ') !== false || strpos($hay, 'цвят') !== false );
...
function gck_attr_group_token( string $label, string $type ) : string {
   ...
   if ( $type === 'color' ) {
    $l = str_replace(
        ['boja', 'barva', 'kolor', 'color', 'colour', ':'],
        ' ',
        $l
    );
} else {
    $l = str_replace(
        ['veličina', 'velicina', 'rozmiar', 'size', ':'],
        ' ',
        $l
    );
}
...
function gck_pl_majice_phrase( int $n, bool $free = false, string $type = 'majica' ) : string {
    if ( $type === 'steznica' ) {
        $m100 = $n % 100;
        if ( $n === 1 ) { $adj = 'darmową'; $noun = 'ortezę'; }
        elseif ( in_array( $n % 10, array( 2, 3, 4 ), true ) && ! in_array( $m100, array( 12, 13, 14 ), true ) ) { $adj = 'darmowe'; $noun = 'ortezy'; }
        else { $adj = 'darmowych'; $noun = 'ortez'; }
        return $free ? ( $adj . ' ' . $noun ) : $noun;
    }
    $m100 = $n % 100;
    if ( $type === 'bokserica' ) {
        if ( $n === 1 || ( in_array( $n % 10, array( 2, 3, 4 ), true ) && ! in_array( $m100, array( 12, 13, 14 ), true ) ) ) {
            $adj = 'darmowe'; $noun = 'bokserki';
        } else {
            $adj = 'darmowych'; $noun = 'bokserek';
        }
    } elseif ( $type === 'carapa' ) {
        // Compression SOCKS (pary skarpet).
        if ( $n === 1 ) {
            $adj = 'darmową'; $noun = 'parę skarpet';
        } elseif ( in_array( $n % 10, array( 2, 3, 4 ), true ) && ! in_array( $m100, array( 12, 13, 14 ), true ) ) {
            $adj = 'darmowe'; $noun = 'pary skarpet';
        } else {
            $adj = 'darmowych'; $noun = 'par skarpet';
        }
    } else {
        if ( $n === 1 ) {
            $adj = 'darmową'; $noun = 'koszulkę';
        } elseif ( in_array( $n % 10, array( 2, 3, 4 ), true ) && ! in_array( $m100, array( 12, 13, 14 ), true ) ) {
            $adj = 'darmowe'; $noun = 'koszulki';
        } else {
            $adj = 'darmowych'; $noun = 'koszulek';
        }
    }
    return $free ? ( $adj . ' ' . $noun ) : $noun;
}
...
function gck_render_bundle_selector() {
    ...
    if (
        ...
        || ( stripos( (string) $product->get_slug(), 'skarpet' ) !== false )
        || ( stripos( (string) $product->get_name(), 'skarpet' ) !== false )
    ) {
    ...
    //  - orto-ortopas                 : single "Rozmiar" attribute, no colour (size selector only).
    ...
    if ( has_term( array( 'orto-cloath' ), 'product_cat', $product_id ) ) {
        $gck_offer_subs = array( 'Zestaw do łazienki', 'Pakiet na cały dom', 'Pakiet dla rodziny i przyjaciół' );
    } else {
        $gck_offer_subs = array();
    }
    $gck_offer_badges = has_term( array( 'orto-cloath', 'orto-cloud' ), 'product_cat', $product_id )
        ? array( 1 => 'NAJPOPULARNIEJSZE', 2 => 'NAJLEPSZA CENA' )
        : array();
    ...
    // extended to 3 garment groups (4 koszulki + 1 bokserki + 1 skarpety). Gated
    // strictly to this category. This site's save/sync collapses product
    // attributes to a single Kolor/Rozmiar, so the 3 groups' colour/size options
...
.color-hnědá {
  ...
  margin-right:6px;
    ...
    if (  !has_term( array( 'orto-cloath', 'orto-hyd', 'orto-cloud', 'orto-starter', 'orto-majice', 'orto-majica', 'orto-bokserice', 'orto-kompresijske-carape', 'orto-ortopas', 'orto-kneefix', 'orto-controlpro', 'orto-bunion', 'orto-fisiorest', 'orto-norikshers', 'orto-noriks-hers', 'orto-majica-darila', 'orto-leak-boxers', 'orto-kompresijske-majice', 'orto-ortopedski-jastuk', 'orto-kidsnest' ), 'product_cat', $product_id )  )   :
    ?>
        <style>
    ...
    <?php
    // SHGIFTS: the skarpety size labels ("39-43" …) are wider than shirt/boxer sizes,
    ...
    <div class="gck-benefits-box">
        ...
        <ul class="gck-benefits-list">
            <?php if ( !has_term( array( 'orto-bokserice', 'orto-bokserice2', 'starter-paketi' ), 'product_cat', $product_id ) ) : ?>
                <li><span class="gck-check">✔</span> <strong>Idealne dopasowanie</strong></li>
            <?php endif; ?>

            <li><strong>✔ Materiały premium i precyzyjny krój</strong></li>
            <li><strong>✔ Komfort bez kompromisów</strong></li>

            <?php if ( has_term( array( 'orto-starter' ), 'product_cat', $product_id ) ) : ?>
                <li style="color: #c00;"><strong>✔ Pakiet startowy dostępny tylko raz na osobę</strong></li>
                <li style="color: #c00;"><strong>✔ Limitowane do 1 000 pakietów </strong></li>
            <?php endif; ?>
        </ul>
        <?php endif; ?>
    ...
    <!--
    ✔ Idealne dopasowanie
✔ Materiały premium i precyzyjny krój
✔ Komfort bez kompromisów
✔ Pakiet startowy dostępny tylko raz na osobę
✔ Limitowane do 1 000 pakietów
    ...
    <div class="gck-top-banner-wrap">
        <?php if ( has_term( array( 'orto-starter' ), 'product_cat', $product_id ) ) : ?>
           ...
           <span>Zamów swój ekskluzywny <br/> pakiet startowy</span>
        </div>
    ...
    <?php if ( has_term( array( 'orto-starter' ), 'product_cat', $product_id ) ) : ?>
        <div class="dev-banner" data-total="1000" data-sold="354">
            <div class="dev-banner__text">
                ...
                <span>Ograniczona ilość <b class="sold">0</b> / <b class="total">0</b> (zbývá <b class="remain">0</b> sztuk)</span>
            </div>
    ...
    <?php if ( $show_countdown ) : ?>
        ...
        <div class="gck-countdown" id="gck-countdown"
            ...
            <div class="gck-countdown__body">
                Promocja ważna tylko dziś — jeszcze <span class="gck-countdown__timer" aria-live="polite">--:--</span>.<?php
                if ( $countdown_stock > 0 ) :
                    ?> Zostało tylko <strong><?php echo esc_html( $countdown_stock ); ?> szt.</strong><?php
                endif; ?> <strong>Pośpiesz się, zanim się skończą!</strong>
            </div>
        </div>
        <script>
          (function(){
    ...
    <div id="bundle-selector" class="bundle-box<?php echo $gck_single_size ? ' is-single-size' : ''; ?><?php echo $gck_no_attrs ? ' is-no-attrs' : ''; ?>" data-split-garments="<?php echo $gck_split_garments ? '1' : '0'; ?>">
        ...
        foreach ( $offers as $offer_id => $data ) :
            ...
            // SHGIFTS: real piece count is 6 (4 koszulki + 1 bokserki + 1 skarpety),
        ...
        ?>
            <label style="position: relative; <?php if ( ( ( $loop_index == 1 || $loop_index == 3 ) && ! $show_group_titles ) || ( ! empty( $gck_offer_badges ) && isset( $gck_offer_badges[ $loop_index ] ) ) ) : ?> margin-top: 25px;  <?php endif; ?>"
                ...
                <?php if ( ! $show_group_titles ) : ?>
                    <?php if ( $loop_index == 1 ) : ?>
                        <div class="gck-popular-badge">Ulubiony 🔥</div>
                    <?php endif; ?>
    ...
    && !has_term( array( 'starter-paketi' ), 'product_cat', $product_id ) 
                ...
                <?php if ( $show_price_highlights ) : ?>
                    ...
                    <span class="gck-per-chip">
                        <?php if ( $per_regular > $gck_per_new ) : ?>
                            <span class="gck-per-old"><?php echo number_format( $per_regular, 2 ); ?> zł</span>
                        <?php endif; ?>
                        <span class="gck-per-new"><?php echo number_format( $gck_per_new, 2 ); ?> zł / szt.</span>
                    </span>
                ...
                <?php else : ?>
                    — <span class="bundle-option-title"><?php echo number_format( $gck_per_new, 2 ); ?> zł / szt.</span>
                <?php endif; ?>
                ...
                <div class="bundle-total-line">
                    ...
                    <span class="line-total"><?php echo number_format( (float) $data['total'], 2 ); ?>zł</span>
                </div>

-->

<div class="bundle-total-line">
    <span style="font-weight:normal;">Razem:</span>

    <?php if ( ! empty($data['regular']) && (float)$data['regular'] > (float)$data['total'] ) : ?>
        <span class="gck-regular-price">
            <?php echo number_format( (float) $data['regular'], 2 ); ?> zł
        </span>
    <?php endif; ?>
    ...
    <span class="line-total"><?php echo number_format( (float) $data['total'], 2 ); ?> zł</span>
                ...
                <div class="bundle-pairs <?php echo $is_default ? '' : 'hidden'; ?>"
                    ...
                    // attributes — "Kolor koszulki"/"Rozmiar koszulki", "Kolor bokserek"/
                    // "Rozmiar bokserek", "Kolor skarpet"/"Rozmiar skarpet" — grouped by
                    ...
                    if ( $gck_split_garments ) {
                        $gck_passes = array(
                            array( 'group' => 0, 'label' => 'Wybierz ' . (int) $pairs . ' ' . gck_pl_majice_phrase( $pairs, false, 'majica' ), 'gratis' => false ),
                            array( 'group' => 1, 'label' => 'Wybierz jeszcze ' . (int) $pairs . ' ' . gck_pl_majice_phrase( $pairs, true, 'bokserica' ), 'gratis' => true ),
                        );
                    } else {
                    ...
                    <?php for ( $i = 1; $i <= $pairs; $i++ ) : ?>
                        <?php if ( $gck_pass['group'] === null && $gck_show_sections && $gck_paid > 0 && $i === 1 ) : ?>
                            <div class="gck-pair-label">Wybierz <?php echo (int) $gck_paid; ?> <?php echo esc_html( gck_pl_majice_phrase( $gck_paid, false, $gck_garment ) ); ?></div>
                        <?php endif; ?>
                        <?php if ( $gck_pass['group'] === null && $gck_show_sections && $gck_free > 0 && $i === ( $gck_paid + 1 ) ) : ?>
                            <div class="gck-pair-label is-gratis">Wybierz jeszcze <?php echo (int) $gck_free; ?> <?php echo esc_html( gck_pl_majice_phrase( $gck_free, true, $gck_garment ) ); ?></div>
                        <?php endif; ?>
                        <div class="bundle-pair">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateBundleUI(selectedRadio) {
        ...
        if (priceElem && total) {
            priceElem.innerHTML = total.replace('.', ',') + ' zł';
        }
    }
 ...
 * total: koszulka 1-4 (colour+size), bokserki (colour+size), skarpety (colour+size).
 * Each line is labelled by inspecting its field keys so the cart / order clearly
 * shows which garment was picked, e.g. "Koszulka: Czarny - L", "Bokserki: Niebieski - M",
 * "Skarpety: Czarny - 39-43".
 */
function gck_build_shgifts_lines( array $pairs_data ) : array {
    ...
    foreach ( $pairs_data as $attrs ) {
        ...
        foreach ( array_keys( $attrs ) as $k ) {
            ...
            if ( strpos( $kl, 'majic' ) !== false )    { $label = 'Koszulka'; break; }
            if ( strpos( $kl, 'bokseric' ) !== false ) { $label = 'Bokserki'; break; }
            if ( strpos( $kl, 'carap' ) !== false )    { $label = 'Skarpety'; break; }
        }
...
function gck_add_cart_item_data( $cart_item_data, $product_id, $variation_id ) {
    ...
    // SHGIFTS (orto-majica-darila): 6 items (4 koszulki + 1 bokserki + 1 skarpety),
...
function gck_shgifts_free_gifts_notice() {
    ...
    // Gifts shown: 1x bokserki + 5x para skarpet. Images + crossed-out reference
    ...
    $gifts = array(
        array( 'label' => '1x bokserki', 'img' => $boxer_img, 'price' => '79,99 zł' ),
    );
    for ( $i = 0; $i < 5; $i++ ) {
        $gifts[] = array( 'label' => '1x para skarpet', 'img' => $socks_img, 'price' => '39,99 zł' );
    }
    ?>
    <style>
      .gck-fg{
    ...
    <div class="gck-fg">
        <div class="gck-fg__head">6 produktów GRATIS do każdego zakupu</div>
