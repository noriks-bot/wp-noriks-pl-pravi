jQuery(function($){
  ...
  function canonicalKey(key){
    // remove WC prefixes and diacritics: e.g. "attribute_Rozmiar" -> "rozmiar"
    ...
    $wrap.find('.ibb-slot').each(function(i){
      ...
      if(filled){
        ...
        for(var k in attr){
          ...
          var ck = canonicalKey(k); // e.g. "attribute_Rozmiar" -> "rozmiar"
          if (ck === 'size' || ck === 'velkost' || ck === 'velicina' || ck === 'rozmiar') {
            size = String(attr[k]).toUpperCase();
            break;
          }
        }
        $slot.find('.ibb-slot-attr').text(size || 'Wybierz koszulkę');
      } else {
        $slot.find('.ibb-slot-attr').text('Wybierz koszulkę');
      }
    ...
    $scope
      ...
      .each(function(){
        ...
        var key =
          ...
          $el.attr('name') || '';
        var ck = canonicalKey(key); // "rozmiar", "size", "velkost", ...

        if (ck !== 'size' && ck !== 'velkost' && ck !== 'velicina' && ck !== 'rozmiar') return;
  ...
  $('.ibb-wrapper').each(function(){
    ...
    $modal.on('click', '.variation-buttons [data-value]', function(){
      ...
      var attrName = group.data('attribute-name'); // e.g. "rozmiar"
    ...
    $modal.on('click','.ibb-add',function(){
      ...
      if(type==='variable'){
        ...
        if(!ok){ $card.find('.ibb-msg').text('Wybierz wariant.'); return; }
        ...
        $.post(IBB.ajax,{action:'ibb_find_variation',nonce:IBB.nonce,product_id:pid,attributes:attrs},function(r){
          if(!r||!r.success||!r.data||!r.data.variation_id){
            $card.find('.ibb-msg').text(r&&r.data&&r.data.message?r.data.message:'Ta wariacja jest niedostępna.');
            return;
          }
          finishSave({product_id:pid,variation_id:r.data.variation_id,attr:attrs,title:title,price_html:r.data.price||price,img:img});
        },'json').fail(function(){ $card.find('.ibb-msg').text('Błąd serwera.'); });
