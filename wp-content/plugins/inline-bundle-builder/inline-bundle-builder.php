 ...
 * Description: Build a bundle in a responsive modal. Saves each slot as numbered meta (e.g., "1: Czarna koszulka - L") in orders. Cart/checkout shows the same lines without a label.
 * Version:     3.1.1-pl
 * Author:      You
 */
...
class IBB_Plugin_SimpleMeta_PL {
    ...
    public function shortcode($atts) {
        ...
        wp_enqueue_style ('ibb-style', plugins_url('ibb.css?dsd', __FILE__), [], '3.1.1-pl');
        wp_enqueue_script('ibb-script', plugins_url('ibb.js?dsds',  __FILE__), ['jquery'], '3.1.1-pl', true);
...
JS;
        ...
        foreach ($ids as $pid) {
            ...
            if ($p->is_type('variable')) {
                ...
                foreach ($vattrs as $tax_raw => $options) {
                    ...
                    $label = wc_attribute_label($tax_norm) ?: wc_attribute_label($tax_raw) ?: 'Opcja';
                    $opts  = [];
        ...
        <div class="ibb-wrapper" data-slots="<?php echo esc_attr($slots); ?>">
          <div class="ibb-slots">
            <?php for ($i=0; $i<$slots; $i++): ?>
              <div class="ibb-slot-wrap">
                <div class="ibb-slot-header"><?php echo sprintf(esc_html__('Wybierz koszulkę %1$d z %2$d', 'ibb'), $i+1, $slots); ?></div>
                <div class="ibb-slot is-empty" data-index="<?php echo $i; ?>" tabindex="0">
                  ...
                  <div class="ibb-slot-info">
                    <span class="ibb-slot-attr"><?php esc_html_e('Wybierz koszulkę','ibb'); ?></span>
          ...
          <div class="ibb-modal" aria-hidden="true">
            <div class="ibb-modal__box" role="dialog" aria-modal="true" aria-label="Budowa zestawu">
              <div class="ibb-modal__head">
                ...
                <div class="ibb-titlebar">
                  <h2 class="ibb-heading"><?php esc_html_e('Wybierz', 'ibb'); ?> <span class="ibb-step-num">1</span> <?php esc_html_e('koszulkę', 'ibb'); ?></h2>
                  <?php if ($bundle_price): ?>
                    <p class="ibb-sub">
                      <?php printf(esc_html__('Dodaj %1$d koszulek i odbierz zestaw za %2$s', 'ibb'), $slots, $bundle_price); ?>
                    </p>
                  <?php endif; ?>
                </div>
              </div>
                        ...
                        <?php if ($c['type'] === 'variable' && !empty($c['attrs'])): ?>
                          <?php foreach ($c['attrs'] as $a): ?>
                            ...
                            <select class="ibb-attr"
                              ...
                              <option value=""><?php echo esc_html__('Wybierz wariant', 'ibb'); ?></option>
                          ...
                          <?php endforeach; ?>
                        <?php endif; ?>
                        <button type="button" class="ibb-add"><?php esc_html_e('Dodaj do zestawu','ibb'); ?></button>
    ...
    public function ajax_find_variation() {
        ...
        // fixing cases where client already sent "attribute_*".
        $attrs = [];
        ...
        if (!$variation_id) {
            wp_send_json_error(['message' => 'Ta wariacja jest niedostępna.']);
        }
    ...
    private function canon_key($k) : string {
        ...
        return $k; // e.g. "rozmiar", "size", "velkost", "velicina", "rozmiartabela"
    }
    ...
    private function slot_line(array $slot) : string {
        ...
        if (!empty($slot['attr']) && is_array($slot['attr'])) {
            foreach ($slot['attr'] as $k => $v) {
                ...
                if (in_array($ck, ['size','velkost','velicina','rozmiar','rozmiartabela'], true)
                    || preg_match('~size|velkost|velicina|rozmiar|rozmiartabela~i', $ck)) {
