 ...
 * Step 2: 6-product grid (after "Nie chcę" or after adding 1 item)
 *
...
foreach ( $order->get_items() as $item ) {
    ...
    $is_majica = ( strpos($cat_str, 'orto-majica') !== false || strpos($cat_str, 'koszulk') !== false || strpos($name, 'koszulk') !== false || strpos($cat_str, 'majic') !== false || strpos($name, 'majic') !== false );
    ...
    // Komplet
    $is_komplet = ( strpos($cat_str, 'komplet') !== false || strpos($cat_str, 'zestaw') !== false || strpos($name, 'zestaw') !== false || strpos($cat_str, 'orto-starter') !== false || strpos($name, 'komplet') !== false || strpos($cat_str, 'komplet') !== false );
...
<div id="order-splash" style="position:fixed;top:0;left:0;width:100%;height:100%;background:#04ac00;z-index:999999;display:flex;flex-direction:column;align-items:center;justify-content:center;transition:opacity 0.6s ease;">
    ...
    <h1 style="color:#fff;font-family:'Roboto',sans-serif;font-size:28px;font-weight:700;margin:0;">Zamówienie przyjęte!</h1>
    <p style="color:rgba(255,255,255,0.85);font-family:'Roboto',sans-serif;font-size:15px;margin:10px 0 0;">Numer zamówienia: #<?php echo $order->get_order_number(); ?></p>
</div>
...
<div class="ty-container">
    ...
    <?php if ( $order->has_status( 'failed' ) ) : ?>
        <div class="ty-success" style="background:#fde8e8;">
            ...
            <h1>Zamówienie nie powiodło się</h1>
            <p>Bank odrzucił transakcję. Spróbuj ponownie.</p>
            <p style="margin-top:16px;">
                <a href="<?php echo esc_url( $order->get_checkout_payment_url() ); ?>" style="display:inline-block;background:#E8450E;color:#fff;padding:12px 32px;text-decoration:none;font-weight:700;">Spróbuj ponownie</a>
            </p>
        </div>
    <?php else : ?>
        ...
        <div class="ty-success">
            ...
            <p>Potwierdzenie zostało wysłane na <?php echo esc_html( $order->get_billing_email() ); ?></p>
            <span class="ty-order-num">Zamówienie #<?php echo $order->get_order_number(); ?></span>
        </div>
            ...
            <div class="ty_upsell_one_wrapper__popup-content">
                ...
                <div class="tyuo_timer" style="position:relative;">
                    <div class="timer_wrapper">
                        <div class="special_offer_txt">Ostatnia szansa – oferta wygasa za</div>
                ...
                <div class="tyuo_middle_section">
                    <div class="sub_title">
                        ...
                        Bez dodatkowej dostawy – wszystko w jednej paczce
                    </div>
                    <div class="clue_text">
                        ...
                        Uzupełnij kombinację i zaoszczędź
                    </div>
                </div>
                ...
                <div class="tyuo_product_section">
                    ...
                    <div class="product_data">
                        ...
                        <div class="right_section_wrapper">
                            ...
                            <div class="product_regular_price" id="ty-upsell-regular"><?php echo number_format($upsell_qty_regular[3], 2, ',', ' '); ?> zł</div>
                            <div class="product_new_sale_price" id="ty-upsell-price"><?php echo number_format($upsell_qty_prices[3], 2, ',', ' '); ?> zł</div>
                        </div>
                    </div>

                    <script>
                    (function(){
                        var prices = <?php echo json_encode(array_map(function($p){ return number_format($p,2,',',' ') . ' zł'; }, $upsell_qty_prices)); ?>;
                        var names = <?php echo json_encode($upsell_qty_names); ?>;
                        var images = <?php echo json_encode($upsell_qty_images); ?>;
                        var regulars = <?php echo json_encode(array_map(function($p){ return number_format($p,2,',',' ') . ' zł'; }, $upsell_qty_regular)); ?>;
                        ...
                        <select class="variation-select" id="ty-variation-select">
                            <?php if ( $upsell_variations ) : ?>
                                ...
                                <option value="<?php echo $v['id']; ?>" <?php selected( strtolower($v['size']), strtolower($customer_size) ); ?>>
                                    Czarna, <?php echo esc_html( $v['size'] ); ?>
                                </option>
                                <?php endforeach; ?>
                            <?php else : ?>
                                <option value="">Czarna, S</option>
                                <option value="">Czarna, M</option>
                                <option value="">Czarna, L</option>
                                <option value="">Czarna, XL</option>
                                <option value="">Czarna, XXL</option>
                            <?php endif; ?>
                        </select>
                    </div>
                    ...
                    <div class="buttons-section">
                        <a class="pass-btn" id="ty-btn-skip">Nie chcę</a>
                        <div class="buy-btn" id="ty-btn-add" data-product-id="<?php echo esc_attr( $upsell_product_id ); ?>">DODAJ DO ZAMÓWIENIA</div>
                    </div>
                </div>
            ...
            <div class="ty-grid-popup">
                <div class="ty-grid-header" style="cursor:pointer;flex-direction:column;position:relative;">
                    ...
                    <div style="display:flex;align-items:center;justify-content:center;gap:10px;margin-bottom:8px;">
                        <div style="font-size:15px;font-weight:400;color:#000;">Oferta specjalna wygasa</div>
                    ...
                    <div style="font-size:20px;font-weight:700;color:#000;line-height:1.3;text-align:center;padding-top:10px;">Dodaj dowolny produkt z 50% rabatem</div>
                </div>
                <div class="ty-section-body open" id="ty-grid-body">
                <div class="ty-section-body-inner" style="padding:0;">
                <div class="ty-grid-trust">
                    ✔ Wszystko wysyłamy w jednej paczce
                </div>
                <div class="ty-grid">
                    ...
                    <div class="ty-grid-item">
                        ...
                        <div class="g-category">BOKSERKI</div>
                        <div class="g-name"><?php echo esc_html( $gp->get_name() ); ?></div>
                        <div class="g-price-old"><?php echo number_format( $gp_price, 2, ',', ' ' ); ?> zł</div>
                        <div class="g-price-new"><?php echo number_format( $gp_sale, 2, ',', ' ' ); ?> zł</div>
                        <?php if ( $gp_vars ) : ?>
                ...
                <div class="buttons-section">
                    <a class="pass-btn" id="ty-grid-close">Nie chcę</a>
                    <div class="buy-btn" id="ty-grid-add-all">DODAJ DO ZAMÓWIENIA</div>
                </div>
        ...
        <div class="ty-section" id="ty-order-items-section">
            <div class="ty-section-header open" onclick="tyToggle(this)">
                <span id="ty-order-items-header">Pozycje zamówienia (<?php echo $order->get_item_count(); ?>)</span>
        ...
        <div class="ty-section">
            ...
            <div class="ty-section-body open">
                <div class="ty-section-body-inner">
                    <div class="ty-row"><span class="ty-row-label">Imię</span><span class="ty-row-value"><?php echo esc_html( $order->get_billing_first_name() . ' ' . $order->get_billing_last_name() ); ?></span></div>
                    ...
                    <div class="ty-row"><span class="ty-row-label">Miasto</span><span class="ty-row-value"><?php echo esc_html( $order->get_billing_postcode() . ' ' . $order->get_billing_city() ); ?></span></div>
...
<?php else : ?>
    <div class="ty-container">
        <div class="ty-success"><h1>Zamówienie</h1>
...
(function(){
    ...
    function tick() {
        if (rem <= 0) {
            ...
            // Auto-open "Pozycje zamówienia" and "Adres dostawy" sections
        ...
        fetch(ajaxUrl, { method: 'POST', body: rfd })
            ...
            .then(function(d) {
                if (d.success) {
                    ...
                    if (headerSpan) {
                        headerSpan.textContent = 'Pozycje zamówienia (' + d.data.item_count + ')';
                    }
        ...
        addBtn.addEventListener('click', function() {
            ...
            fetch(ajaxUrl, { method: 'POST', body: fd })
                ...
                .catch(function() {
                    ...
                    addBtn.textContent = 'DODAJ DO ZAMÓWIENIA';
                });
        });
    }
        ...
        overlay.querySelectorAll('.g-select-btn').forEach(function(btn) {
            btn.addEventListener('click', function() {
                ...
                if (el.classList.contains('selected')) {
                    ...
                    el.textContent = 'WYBIERZ';
                    el.style.background = '#000';
                } else {
                    el.classList.add('selected');
                    el.innerHTML = '✔ WYBRANE <span style="font-size:10px;opacity:0.7;margin-left:4px;">usuń</span>';
                    el.style.background = '#2E7D32';
                }
            });
        });

        // "Nie chcę" — close all, show summary without upsells
        document.getElementById('ty-grid-close').addEventListener('click', closeAll);

        // "DODAJ DO ZAMÓWIENIA" — add all selected items, then close
                ...
                fetch('<?php echo admin_url("admin-ajax.php"); ?>', { method: 'POST', body: rfd })
                    ...
                    .then(function(d2) {
                        if (d2.success) {
                            ...
                            var headerSpan = document.getElementById('ty-order-items-header');
                            if (headerSpan) {
                                headerSpan.textContent = 'Pozycje zamówienia (' + d2.data.item_count + ')';
                            }
                        }
                    });
            } else {
