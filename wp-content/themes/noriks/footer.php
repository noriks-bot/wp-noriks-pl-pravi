...
<!-- suport section -->
	...
	if ( ! $nf_brand ) {
		$nf_brand = 'NORIKS projektuje ponadczasowe ubrania dla mocniejszej sylwetki — dłuższe, wygodniejsze i starannie dopracowane tam, gdzie ma to największe znaczenie.';
	}
	$nf_bg      = get_field( 'footer_bg_image', 'option' ); // background photo behind the brand band
	$nf_legal   = get_field( 'footer_legal_text', 'option' );
	if ( ! $nf_legal ) {
		$nf_legal = 'NORIKS BRAND. Wszelkie prawa zastrzeżone. Ceny zawierają VAT. Zdjęcia mają charakter poglądowy.';
	}
	?>
		...
		<section class="nf-signup">
			<div class="nf-wrap nf-signup-grid">
				<div class="nf-signup-copy">
					<h2 class="nf-signup-title">Dołącz do rodziny NORIKS</h2>
					<p class="nf-signup-sub">Ekskluzywne oferty i zapowiedzi nowych produktów — dowiedz się pierwszy.</p>
				</div>
				<div class="nf-signup-form-col">
					<form class="nf-signup-form" data-klaviyo-list="<?php echo esc_attr( $nf_klaviyo_list ); ?>" novalidate>
						<label class="nf-field-label" for="nf-email">Twój e-mail *</label>
						<div class="nf-field-row">
							<input type="email" id="nf-email" name="email" class="nf-input" placeholder="Twój adres e-mail" required autocomplete="email">
							<button type="submit" class="nf-btn nf-btn-light">Zapisz się</button>
						</div>
						<p class="nf-signup-note">
							Zapisując się, wyrażasz zgodę na otrzymywanie e-maili. Możesz zrezygnować w każdej chwili.
						...
						<div class="nf-link-col">
							...
							if ( $nf_col2_l ) :
								foreach ( $nf_col2_l as $item ) :
									if ( false !== strpos( (string) $item['link'], '/instrukcje' ) ) { $nf_has_upute = true; }
									?>
								<a href="<?php echo esc_url( $item['link'] ); ?>"><?php echo esc_html( $item['text'] ); ?></a>
							<?php endforeach; endif; ?>
							<?php if ( ! $nf_has_upute ) : ?>
								<a href="/pl/instrukcje/">Instrukcja obsługi</a>
							<?php endif; ?>
						</div>
					<?php endif; ?>
		...
		<div class="nf-wrap nf-meta">
			<div class="nf-meta-left">
				<span class="nf-region">
					<img loading="lazy" decoding="async" src="https://static.devit.software/countries/flags/rectangle/pl.svg" alt="" class="nf-flag">
					Polska (PL)
				</span>
				<span class="nf-copy">© <?php echo date( 'Y' ); ?> NORIKS. Wszelkie prawa zastrzeżone.</span>
			</div>
	...
	(function () {
		...
		form.addEventListener('submit', function (e) {
			...
			if (!email || email.indexOf('@') === -1) {
				msg.textContent = 'Podaj prawidłowy adres e-mail.'; msg.className = 'nf-signup-msg is-error'; return;
			}
			if (!list || list === 'REPLACE_LIST_ID') {
				msg.textContent = 'Newsletter nie jest jeszcze połączony (brakuje Klaviyo List ID).'; msg.className = 'nf-signup-msg is-error'; return;
			}
			msg.textContent = 'Zapisywanie…'; msg.className = 'nf-signup-msg';
			...
			fetch('https://manage.kmail-lists.com/ajax/subscriptions/subscribe', {
				...
				if (d && d.success) {
					msg.textContent = 'Dziękujemy! Sprawdź e-mail, aby potwierdzić.'; msg.className = 'nf-signup-msg is-ok';
					form.reset();
				} else {
					msg.textContent = (d && d.errors && d.errors[0]) ? d.errors[0] : 'Coś poszło nie tak, spróbuj ponownie.';
					msg.className = 'nf-signup-msg is-error';
				}
			}).catch(function () {
				msg.textContent = 'Coś poszło nie tak, spróbuj ponownie.'; msg.className = 'nf-signup-msg is-error';
			});
		});
	})();
	</script>
