 ...
 *
 * Okvir se prikazuje ODMAH ISPOD gumba "Dodaj u košaricu" i nudi 4x Zielone bokserki
	...
	acf_add_local_field_group( array(
		...
		'fields' => array(
			array(
				...
				'label'        => 'Pokaż upsell pod przyciskiem (4x Zielone bokserki)',
			...
			array(
				...
				'label'        => 'Pokaż upsell #3 pod przyciskiem (koszulka kompresyjna NORIKS FIT)',
				'name'         => 'noriks_pp_upsell3',
				'type'         => 'true_false',
				'instructions' => 'Trzecia ramka upsell: koszulka kompresyjna NORIKS FIT, klient wybiera rozmiar. Niezależna od dwóch pierwszych przełączników.',
				'ui'           => 1,
			),
		),
		'location'   => array(
...
function noriks_pp_upsell_config() {
	return apply_filters( 'noriks_pp_upsell_config', array(
		'product_id' => 2744,                    // Zelene Bokserice (varijabilni proizvod)
		'qty'        => 4,                       // uvijek 4 komada, iste veličine
		'total'      => 79,                   // ista cijena kao thank you upsell (4 komada)
		'title'      => '4x Zielone bokserki',
		'desc'       => 'Oddychające i miękkie — dodaj je do zamówienia z rabatem %s%%.', // %s = izracunati popust
		'size_attr'  => 'Rozmiar',
...
function noriks_pp_upsell_render() {
	...
	?>
	<div class="npu-wrap">
		<span class="npu-label">Kup razem i oszczędzaj:</span>
			...
			<div class="npu-grid">
				...
				<div class="npu-actions">
					<label class="npu-check">
						...
						<span class="npu-check-text">Dodaj do zakupu</span>
					</label>

					<select class="npu-size" name="noriks_pp_upsell_size" aria-label="Rozmiar bokserek">
...
function noriks_pp_upsell2_config() {
	return apply_filters( 'noriks_pp_upsell2_config', array(
		...
		'total'      => 69, // cijena cijelog paketa (2 komada)
		'title'      => '1x Czarna + 1x Szara koszulka',
		'desc'       => 'Dwie podstawowe koszulki w zestawie — dodaj je do zamówienia z rabatem %s%%.',
		'size_attr'  => 'Rozmiar',
		'sku'        => 'NORIKS-SHIRTS-BLACK-GRAY-2-PACK-UPSELL',
		'image'      => get_template_directory_uri() . '/img/upsell/upsell-2x-majici.png',
		'label'      => 'Kup razem i oszczędzaj:',
		'add_text'   => 'Dodaj do zakupu',
		'aria'       => 'Rozmiar koszulek',
	) );
}
...
function noriks_pp_upsell2_maybe_add( $cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data ) {
	...
	$lines  = array(
		( $prod_a ? $prod_a->get_name() : 'Czarna' ) . ' - ' . $size,
		( $prod_b ? $prod_b->get_name() : 'Szara koszulka' ) . ' - ' . $size,
	);
...
function noriks_pp_upsell3_config() {
	return apply_filters( 'noriks_pp_upsell3_config', array(
		'product_id' => 5470,   // NORIKS FIT kompresijska majica (jednostavan proizvod)
		'total'      => 79,  // cijena za 1 komad
		'title'      => 'Koszulka kompresyjna NORIKS FIT',
		'desc'       => 'Modeluje sylwetkę i wspiera postawę — dodaj ją do zamówienia z rabatem %s%%.',
		'size_attr'  => 'Rozmiar',
...
function noriks_pp_upsell3_order_item_meta( $item, $cart_item_key, $values, $order ) {
	if ( ! empty( $values['_noriks_pp_upsell3'] ) ) {
		...
		if ( ! empty( $values['_noriks_pp_upsell_size'] ) ) {
			$item->update_meta_data( 'Rozmiar', sanitize_text_field( $values['_noriks_pp_upsell_size'] ) );
		}
	}
}
