...
function noriks_ajax_add_to_cart() {
    ...
    if ($cart_item_key) {
        ...
        $msg = !empty($notices) ? strip_tags($notices[0]['notice'] ?? $notices[0]) : 'Błąd podczas dodawania';
        wc_clear_notices();
...
function noriks_upsell_modal_markup() {
        ...
        <div class="noriks-modal">
            ...
            <div class="noriks-modal-body">
                ...
                <div class="noriks-modal-qty-row">
                    <span class="noriks-attr-label">ILOŚĆ</span>
                ...
                <div id="noriks-modal-error" class="noriks-modal-error" style="display:none;">Wybierz wszystkie opcje</div>
                <button id="noriks-modal-add" class="noriks-modal-add-btn">DODAJ DO KOSZYKA</button>
            </div>
    ...
    (function($) {
        ...
        function renderModal() {
            ...
            $('#noriks-modal-add').text('DODAJ DO KOSZYKA').removeClass('adding added');
        ...
        $(document).on('click', '#noriks-modal-add', function() {
            ...
            if (!variation) {
                $('#noriks-modal-error').text('Ta kombinacja nie jest dostępna').show();
                return;
            }

            if (!variation.is_in_stock) {
                $('#noriks-modal-error').text('Brak w magazynie').show();
                return;
            }
            ...
            $.post(woocommerce_params.ajax_url, data, function(res) {
                ...
                if (res.success !== false && res.fragments) {
                    $btn.removeClass('adding').addClass('added').text('✓ DODANE!');
                    ...
                    $btn.removeClass('adding').text('DODAJ DO KOSZYKA');
                    var msg = (res.data && res.data.message) ? res.data.message : 'Błąd podczas dodawania';
                ...
                $btn.removeClass('adding').text('DODAJ DO KOSZYKA');
                $('#noriks-modal-error').text('Błąd podczas dodawania').show();
            });
        });
